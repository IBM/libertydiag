...
@WebServlet(Constants.CONTEXT_SERVLET + "AllocateObject")
public class AllocateObject extends BaseServlet {
	...
	protected void doWork(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
		...
		boolean retainData = requestBoolean(request, "retainData", DefaultRetainData);
		boolean retainDataGc = requestBoolean(request, "retainDataGc", DefaultRetainDataGc);
		int oomcount = 0;
		...
		try {
			for (int i = 1; i <= iterations; i++) {
				try {
					println(out, "Iteration " + i + ": Allocating byte array of " + size + " bytes... [" + holder.size()
							+ "]", true, false);
					byte[] data = new byte[size];
					holder.add(data);
					println(out, "allocation complete of " + data.length + " bytes.");

...
import com.example.util.Chopstick;
import com.example.util.Constants;
import com.example.util.Philosopher;

@WebServlet(Constants.CONTEXT_SERVLET + "Deadlocker")
public class Deadlocker extends BaseServlet {
	...
	protected void doWork(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
		...
		/*
		 * This is the original as coded. It will hang since none of these threads ever
		 * finish. Seems kind of pointless, so commenting out.
		 */

		/*
		 * try { p1.join(); } catch (Throwable t) { t.printStackTrace(); } try {
		 * p2.join(); } catch (Throwable t) { t.printStackTrace(); } try { p3.join(); }

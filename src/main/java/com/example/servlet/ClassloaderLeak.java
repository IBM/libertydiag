...
@WebServlet(Constants.CONTEXT_SERVLET + "ClassloaderLeak")
	...
	protected void doWork(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
		...
		try {
			...
			if (LEAK_TO_LIST) {
				...
				Constructor<?> constructor = (JAVA_SURGERY_JAR_FILE == null ? c : LeakedThread.class)
						.getConstructors()[0];
				Thread thread = (Thread) constructor.newInstance(JAVA_SURGERY_JAR_FILE == null ? ccl : c);
				thread.setDaemon(true);
				thread.setName(
						LeakedThread.class.getSimpleName() + "0x" + Long.toHexString(System.identityHashCode(thread)));
				thread.start();
